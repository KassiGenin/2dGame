...
import io.github.epitech_game.Enemy;
import io.github.epitech_game.Fly;
import io.github.epitech_game.GameMap;
import io.github.epitech_game.Hero;
import io.github.epitech_game.Main;
import io.github.epitech_game.SpawnDirection;
import io.github.epitech_game.Wizard;
...
public class GameScreen implements Screen {
    ...
    public GameScreen(Main game, SpawnDirection spawnDirection, GameMap map) {
        this.game = game;
        this.spawnDirection = spawnDirection;
        this.currentMap = map;
        initialize();
    ...
    private void initialize() {
        try {
            // Load Tiled map based on currentMap
            TmxMapLoader mapLoader = new TmxMapLoader();
            map = mapLoader.load(currentMap.getMapPath());
            mapRenderer = new OrthogonalTiledMapRenderer(map);
            Gdx.app.log("GameScreen", "Successfully loaded " + currentMap.getMapPath());
        } catch (Exception e) {
            Gdx.app.error("GameScreen", "Failed to load " + currentMap.getMapPath(), e);
            Gdx.app.exit();
            return;
        }
        ...
        try {
            collisionLayer = (TiledMapTileLayer) map.getLayers().get("collisions");
            if (collisionLayer == null) {
                Gdx.app.error("GameScreen", "'collisions' layer not found in " + currentMap.getMapPath());
                Gdx.app.exit();
                return;
            }
            Gdx.app.log("GameScreen", "'collisions' layer successfully retrieved");
    ...
    private void setHeroSpawnPosition() {
        ...
        switch (spawnDirection) {
            case FROM_TOP:
                spawnX = mapWidth / 2f - heroWidth / 2f;
                spawnY = mapHeight - heroHeight - 10; // Slight padding from the top
                break;
            case FROM_BOTTOM:
                spawnX = mapWidth / 2f - heroWidth / 2f;
                spawnY = 10; // Slight padding from the bottom
                break;
            case FROM_LEFT:
                spawnX = 10; // Slight padding from the left
                spawnY = mapHeight / 2f - heroHeight / 2f;
                break;
            case FROM_RIGHT:
                spawnX = mapWidth - heroWidth - 10; // Slight padding from the right
                spawnY = mapHeight / 2f - heroHeight / 2f;
                break;
            case CENTER:
            default:
    ...
    /**
     * Initializes enemies based on the current map.
     */
    private void initializeEnemies() {
        enemies = new ArrayList<>();
        if (currentMap == GameMap.WORLD_2) {
            // Initialize enemies for World 2
            Fly fly = new Fly(hero);
            fly.setPosition(300, 300);
            enemies.add(fly);
            Gdx.app.log("GameScreen", "Enemy (Fly) initialized at (300,300)");
        } else if (currentMap == GameMap.OVERWORLD) {
            // Initialize enemies for Overworld
            Wizard wizard = new Wizard(hero);
            wizard.setPosition(500, 500);
            enemies.add(wizard);
            Gdx.app.log("GameScreen", "Enemy (Wizard) initialized at (500,500)");
        } else if (currentMap == GameMap.WORLD_3) {
            // Initialize enemies for World 3
            // Add enemies for World 3
        }
    }
    ...
    /**
     * Checks if the hero has interacted with any gates to transition to a new map.
     */
    private void checkForGateTransitions() {
        MapLayer gateLayer = map.getLayers().get("Gates");
        if (gateLayer != null) {
            MapObjects gateObjects = gateLayer.getObjects();
            Rectangle heroBounds = hero.getBounds();

            for (MapObject object : gateObjects) {
                if (object instanceof RectangleMapObject) {
                    RectangleMapObject rectangleObject = (RectangleMapObject) object;
                    Rectangle gateRect = rectangleObject.getRectangle();

                    if (heroBounds.overlaps(gateRect)) {
                        String toMapName = rectangleObject.getProperties().get("toMap", String.class);
                        String directionName = rectangleObject.getProperties().get("spawnDirection", String.class);

                        if (toMapName != null && directionName != null) {
                            GameMap nextMap = GameMap.valueOf(toMapName.toUpperCase());
                            SpawnDirection nextSpawnDirection = SpawnDirection.valueOf(directionName.toUpperCase());
                            transitionToNewScreen(nextSpawnDirection, nextMap);
                            break;
                        }
                    }
                }
    ...
    /**
     * Handles the transition to a new screen with the specified spawn direction and map.
     *
     * @param direction The direction from which the hero is entering the new screen.
     * @param nextMap   The next map to load.
     */
    private void transitionToNewScreen(SpawnDirection direction, GameMap nextMap) {
        // Dispose current screen resources if necessary
        dispose();

        // Set the new screen
        game.setScreen(new GameScreen(game, direction, nextMap));
    }
